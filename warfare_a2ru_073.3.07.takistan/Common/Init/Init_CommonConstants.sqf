...
with missionNamespace do {
	...
	if (isNil "WFBE_C_ARTILLERY_COMPUTER") then {WFBE_C_ARTILLERY_COMPUTER = 1}; //--- Enable Artillery Computer (0: Disabled, 1: Enabled on Upgrade, 2: Enabled).
	...
	WFBE_C_ARTILLERY_INTERVALS = [300, 250, 200, 150]; //--- Delay between each fire mission for each upgrades.
	
	//--- Base
	if (isNil "WFBE_C_BASE_AREA") then {WFBE_C_BASE_AREA = 2}; //--- Force the bases to be grouped by areas.
	if (isNil "WFBE_C_BASE_ALLIES") then {WFBE_C_BASE_ALLIES = 0}; //--- Allies Parameters (0 Disabled, 1 West, 2 East, 3 both).
	if (isNil "WFBE_C_BASE_DEFENSE_MAX_AI") then {WFBE_C_BASE_DEFENSE_MAX_AI = 30}; //--- Maximum AIs that will be able to man defense within the barracks area.
	...
	if (isNil "WFBE_C_BASE_PATROLS_INFANTRY") then {WFBE_C_BASE_PATROLS_INFANTRY = 0}; //--- Enable AI Squad patrol in base from Barracks.
	...
	if (isNil "WFBE_C_BASE_STARTING_DISTANCE") then {WFBE_C_BASE_STARTING_DISTANCE = 5000}; //--- Sides need at last xkm of distance between them.
	...
	WFBE_C_BASE_HQ_REPAIR_PRICE = 20000; //--- HQ Repair price.
	
	//--- Camps.
	if (isNil "WFBE_C_CAMPS_CREATE") then {WFBE_C_CAMPS_CREATE = 1}; //--- Create the camp models.
	WFBE_C_CAMPS_CAPTURE_BOUNTY = 100; //--- Bounty received by player whenever he capture a camp.
	WFBE_C_CAMPS_CAPTURE_RATE = 2;
	WFBE_C_CAMPS_CAPTURE_RATE_MAX = 5;
	WFBE_C_CAMPS_RANGE = 10;
	WFBE_C_CAMPS_RANGE_PLAYERS = 5;
	WFBE_C_CAMPS_REPAIR_DELAY = 35;
	WFBE_C_CAMPS_REPAIR_PRICE = 2500;
	...
	if (isNil "WFBE_C_ECONOMY_INCOME_SYSTEM") then {WFBE_C_ECONOMY_INCOME_SYSTEM = 4}; //--- Income System (1:Full, 2:Half (Half -> 120 SV Town = 60$ / 60SV), 3: Commander System, 4: Commander System: Full)
	...
	WFBE_C_ECONOMY_INCOME_COEF = 10; //--- Town Multiplicator Coefficient (SV * x).
	WFBE_C_ECONOMY_INCOME_DIVIDED = 2.75; //--- Prevent commander from being a millionaire, and add the rest to the players pool.
	WFBE_C_ECONOMY_INCOME_PERCENT_MAX = 15; //--- Commander may set income up to x%.
	WFBE_C_ECONOMY_SUPPLY_TIME_INCREASE_DELAY = 80; //--- Increase SV delay.
	
	//--- Environment.
	if (isNil "WFBE_C_ENVIRONMENT_FAST_TIME") then {WFBE_C_ENVIRONMENT_FAST_TIME = 0}; //--- Fast Time rates (where x is the minute # skiped each minute)
	...
	if (isNil "WFBE_C_ENVIRONMENT_WEATHER") then {WFBE_C_ENVIRONMENT_WEATHER = 0}; //--- Weather Type, 0: Clear, 1: Cloudy, 2: Rainy, 3: Dynamic)
	if (isNil "WFBE_C_ENVIRONMENT_WEATHER_VOLUMETRIC") then {WFBE_C_ENVIRONMENT_WEATHER_VOLUMETRIC = 0}; //--- Enable volumetric clouds.
	WFBE_C_ENVIRONMENT_FAST_TIME_VALUES = [0, 4.8, 5.052631579, 5.333333333, 5.647058824, 6, 6.4, 6.857142857, 7.384615385, 8, 8.727272727, 9.6, 10.66666667, 12]; //--- Fast time values, this shall match the amount of values in rsc\parameters.hpp
	...
	WFBE_C_GAMEPLAY_VOTE_TIME = if (WF_Debug) then {8} else {60};
	
	//--- Modules.
	if (isNil "WFBE_C_MODULE_BIS_BAF") then {WFBE_C_MODULE_BIS_BAF = 1}; //--- Enable BAF content.
	if (isNil "WFBE_C_MODULE_BIS_HC") then {WFBE_C_MODULE_BIS_HC = 1}; //--- Enable High Command.
	...
	if (isNil "WFBE_C_MODULE_WFBE_MISSIONS") then {WFBE_C_MODULE_WFBE_MISSIONS = 1}; //--- Enable WF BE Secondary Missions.
	if (isNil "WFBE_C_MODULE_UPSMON") then {WFBE_C_MODULE_UPSMON = 0}; //--- Enable UPSMON AI.
	WFBE_C_MODULES_UPSMON_TOWN_AREA = [500,500]; //--- Defines the area of operation for UPSMON AI in a town.
	
	//--- Players.
	if (isNil "WFBE_C_PLAYERS_AI_MAX") then {WFBE_C_PLAYERS_AI_MAX = 14}; //--- Max AI allowed on each player groups.
	WFBE_C_PLAYERS_BOUNTY_CAPTURE = 5000;
	WFBE_C_PLAYERS_BOUNTY_CAPTURE_ASSIST = 5000;
	WFBE_C_PLAYERS_BOUNTY_CAPTURE_MISSION = 5000;
	WFBE_C_PLAYERS_BOUNTY_CAPTURE_MISSION_ASSIST = 5000;
	WFBE_C_PLAYERS_COMMANDER_BOUNTY_CAPTURE_COEF = 200;
	WFBE_C_PLAYERS_COMMANDER_SCORE_BUILD = 1;
	WFBE_C_PLAYERS_COMMANDER_SCORE_CAPTURE = 1;
	WFBE_C_PLAYERS_COMMANDER_SCORE_UPGRADE = 1;
	WFBE_C_PLAYERS_GEAR_SELL_COEF = 0.05; //--- Sell price of the gear: item price * x (800 * 0.5 = 400)
	WFBE_C_PLAYERS_GEAR_VEHICLE_RANGE = 50; //--- Possible to buy gear in vehicle if that one is within that range.
	WFBE_C_PLAYERS_HALO_HEIGHT = 100; //--- Distance above which units are able to perform an HALO jump.
	WFBE_C_PLAYERS_MARKER_DEAD_DELAY = 60; //--- Time that a marker remain on a dead unit.
	WFBE_C_PLAYERS_MARKER_TOWN_RANGE = 0.8; //--- A town marker is updated (SV) on map if a unit is within the range (town range * coef).
	WFBE_C_PLAYERS_OFFMAP_TIMEOUT = 30; //--- Player may remain x second outside of the map before being killed.
	WFBE_C_PLAYERS_PENALTY_TEAMKILL = 500; //--- Teamkill penalty.
	...
	WFBE_C_RESPAWN_RANGES = [200, 300, 400, 500];
	...
	if (isNil "WFBE_C_STRUCTURES_CONSTRUCTION_MODE") then {WFBE_C_STRUCTURES_CONSTRUCTION_MODE = 0}; //--- Structures construction mode (0: Time, 1: Workers).
	if (isNil "WFBE_C_STRUCTURES_HQ_COST_DEPLOY") then {WFBE_C_STRUCTURES_HQ_COST_DEPLOY = 100}; //--- HQ Deploy / Mobilize Price.
	if (isNil "WFBE_C_STRUCTURES_HQ_RANGE_DEPLOYED") then {WFBE_C_STRUCTURES_HQ_RANGE_DEPLOYED = 120}; //--- HQ Deploy / Mobilize Price.
	...
	WFBE_C_STRUCTURES_COMMANDCENTER_RANGE = 50000; //--- Command Center Range.
	...
	WFBE_C_STRUCTURES_SALE_DELAY = 240; //--- Building is sold after x seconds.
	WFBE_C_STRUCTURES_SALE_PERCENT = 40; //--- When a structure is sold, x% of supply goes back to the side.
	WFBE_C_STRUCTURES_SERVICE_POINT_RANGE = 50;
	WFBE_C_STRUCTURES_WORKERS_DISTANCE = 30; //--- Minimal Distance in order to start building.
	WFBE_C_STRUCTURES_WORKERS_MAX = 10; //--- Maximum Workers per side.
	WFBE_C_STRUCTURES_WORKERS_PRICE = 500; //--- Worker Price
	WFBE_C_STRUCTURES_WORKERS_RADIUS = 650; //--- Maximal Traveling distance.
	WFBE_C_STRUCTURES_WORKERS_RATIO = 2; //--- Build Speed Ratio.
	WFBE_C_STRUCTURES_WORKERS_REPAIR = 0.0075; //--- Health Point fixed during a repair.
	if (WF_A2_Vanilla) then {
		WFBE_C_BASE_COIN_DISTANCE_MIN = 8;
		WFBE_C_BASE_COIN_GRADIENT_MAX = 4;
	} else {
		WFBE_C_BASE_COIN_DISTANCE_MIN = 24;
		WFBE_C_BASE_COIN_GRADIENT_MAX = 4;
	};
	
	//--- Towns.
	if (isNil "WFBE_C_TOWNS_AMOUNT") then {WFBE_C_TOWNS_AMOUNT = 3}; //--- Amount of towns (0: Very small, 1: Small, 2: Medium, 3: Large, 4: Full).
	...
	if (isNil "WFBE_C_TOWNS_CAPTURE_MODE") then {WFBE_C_TOWNS_CAPTURE_MODE = 1}; //--- Town capture mode (0: Normal, 1: Threshold, 2: All Camps).
	if (isNil "WFBE_C_TOWNS_CONQUEST_MODE") then {WFBE_C_TOWNS_CONQUEST_MODE = 0}; //--- Town Conquest mode (0: None, 1: Territoriality).
	if (isNil "WFBE_C_TOWNS_DEFENDER") then {WFBE_C_TOWNS_DEFENDER = 1}; //--- Town defender Difficulty (0: Disabled, 1: Light, 2: Medium, 3: Hard, 4: Insane).
	if (isNil "WFBE_C_TOWNS_OCCUPATION") then {WFBE_C_TOWNS_OCCUPATION = 1}; //--- Town occupation Difficulty (0: Disabled, 1: Light, 2: Medium, 3: Hard, 4: Insane).
	...
	WFBE_C_TOWNS_CAPTURE_RATE = 1;
	WFBE_C_TOWNS_CAPTURE_THRESHOLD_RANGE = 50;
	...
	WFBE_C_TOWNS_DETECTION_RANGE_AIR = 200; //--- Detect Air if > x
	...
	WFBE_C_TOWNS_PURCHASE_RANGE = 50;
	...
	WFBE_C_TOWNS_UNITS_SPAWN_CAPTURE_DELAY = 600; //--- If x seconds has elapsed since a town last capture, units may spawn again during that town capture.
	...
	if (isNil "WFBE_C_UNITS_EMPTY_TIMEOUT") then {WFBE_C_UNITS_EMPTY_TIMEOUT = 600}; //--- Lifespan of an empty vehicle.
	...
	if (WFBE_C_RESPAWN_PENALTY>0) then 
		{
			WFBE_C_UNITS_BOUNTY_COEF = 0.65; 			//--- Bounty is the unit price * coef.
		}
		else
		{
			WFBE_C_UNITS_BOUNTY_COEF = 0.65; 			//--- Bounty is the unit price * coef.
	...
	WFBE_C_UNITS_SALVAGER_SCAVENGE_RANGE = 20;
	WFBE_C_UNITS_SALVAGER_SCAVENGE_RATIO = 30; //--- Salvager Sell %.
	WFBE_C_UNITS_SKILL_DEFAULT = 0.7;
	WFBE_C_UNITS_SUPPORT_RANGE = 70; //--- Action range for repair/rearm/refuel.
	WFBE_C_UNITS_SUPPORT_HEAL_PRICE = 125;
	WFBE_C_UNITS_SUPPORT_HEAL_TIME = 25;
	WFBE_C_UNITS_SUPPORT_REARM_PRICE = 14;
	WFBE_C_UNITS_SUPPORT_REARM_TIME = 30;
	WFBE_C_UNITS_SUPPORT_REFUEL_PRICE = 16;
	WFBE_C_UNITS_SUPPORT_REFUEL_TIME = 20;
	WFBE_C_UNITS_SUPPORT_REPAIR_PRICE = 2;
	WFBE_C_UNITS_SUPPORT_REPAIR_TIME = 40;
	...
	switch (true) do {
		case (WF_A2_Vanilla): {
			...
			if (isNil "WFBE_C_UNITS_FACTION_WEST") then {WFBE_C_UNITS_FACTION_WEST = 1}; //--- West Faction.
			WFBE_C_UNITS_FACTIONS_EAST = ['INS','RU']; //--- East Factions.
			WFBE_C_UNITS_FACTIONS_GUER = ['GUE']; //--- Guerilla Factions.
			WFBE_C_UNITS_FACTIONS_WEST = ['CDF','USMC']; //--- West Factions.
		};
		case (WF_A2_Arrowhead): {
			if (isNil "WFBE_C_UNITS_FACTION_EAST") then {WFBE_C_UNITS_FACTION_EAST = 0}; //--- East Faction.
			if (isNil "WFBE_C_UNITS_FACTION_GUER") then {WFBE_C_UNITS_FACTION_GUER = 1}; //--- Guerilla Faction.
			if (isNil "WFBE_C_UNITS_FACTION_WEST") then {WFBE_C_UNITS_FACTION_WEST = 0}; //--- West Faction.
			WFBE_C_UNITS_FACTIONS_EAST = ['TKA']; //--- East Factions.
			WFBE_C_UNITS_FACTIONS_GUER = ['PMC','TKGUE']; //--- Guerilla Factions.
			WFBE_C_UNITS_FACTIONS_WEST = ['US']; //--- West Factions.
		};
		case (WF_A2_CombinedOps): {
			if (isNil "WFBE_C_UNITS_FACTION_EAST") then {WFBE_C_UNITS_FACTION_EAST = 1}; //--- East Faction.
			if (isNil "WFBE_C_UNITS_FACTION_GUER") then {WFBE_C_UNITS_FACTION_GUER = 2}; //--- Guerilla Faction.
			if (isNil "WFBE_C_UNITS_FACTION_WEST") then {WFBE_C_UNITS_FACTION_WEST = 1}; //--- West Faction.
	...
	WFBE_C_WEST_COLOR = "ColorBlue";
	WFBE_C_EAST_COLOR = "ColorRed";
	WFBE_C_GUER_COLOR = "ColorGreen";
	WFBE_C_CIV_COLOR = "ColorYellow";
	WFBE_C_UNKNOWN_COLOR = "ColorBlack";
	...
	if (WFBE_C_ENVIRONMENT_FAST_TIME != 0) then {
		WFBE_C_ENVIRONMENT_FAST_TIME = WFBE_C_ENVIRONMENT_FAST_TIME_VALUES select WFBE_C_ENVIRONMENT_FAST_TIME;
	};

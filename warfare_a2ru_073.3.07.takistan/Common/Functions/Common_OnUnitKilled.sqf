...
_type=typeOf _killed;
	if (_type in UAV_VehicleTypes) then
	{
		_unit call Func_UAV_DeletePilot;		
	}
	else
	{
		if (_type in UAV_TerminalTypes) then
		{
			_UAV=_killed getVariable "uav_active";						
			if (!(isNull _UAV)) then
			{
...
["INFORMATION", Format ["Common_OnUnitKilled.sqf: [%1] [%2] has been killed by [%3].", _killed_side, _killed, _killer]] Call WFBE_CO_FNC_LogContent;
	...
	if (_killer_side != _killed_side) then { //--- Normal kill.
		if (isPlayer (leader _killer_group)) then { //--- The team is lead by a player.
			...
			if ((missionNamespace getVariable "WFBE_C_UNITS_BOUNTY") > 0) then { //--- Award the bounty if needed.
				...
				if (vehicle _killer != _killer && alive (vehicle _killer)) then { //--- Kill assist (players in the same vehicle).
					{if (alive _x && isPlayer _x) then {[getPlayerUID(_x), "AwardBounty", [_objectType, true]] Call WFBE_CO_FNC_SendToClients}} forEach ((crew (vehicle _killer)) - [_killer, player]);
				};
			};

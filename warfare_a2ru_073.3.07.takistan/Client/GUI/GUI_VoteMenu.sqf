...
while {alive player && dialog} do {
	...
	_playerCount = 0;
	{
		if (isPlayer (leader _x)) then
        {
            ...
            // Prodavec: based on Bomba1 code snippet, set green color for player(s) which voted for comm.
            //hint format ["_vote:\n%1\n\n_voteArray:\n%2\n\n_x:\n%3", _vote, _voteArray, _x];
            //sleep 0.1;
            if (_vote > 0) then
            {
                // Voted players
                lnbSetColor [500100, [_forEachIndex + 1, 0], [0.1, 0.7, 0.1, 1]];
                lnbSetColor [500100, [_forEachIndex + 1, 1], [0.1, 0.7 ,0.1, 1]];
            }
            else
            {
                // Not voted players
                lnbSetColor [500100, [_forEachIndex + 1, 0], [1.0, 1.0, 1.0, 1]];
                lnbSetColor [500100, [_forEachIndex + 1, 1], [1.0, 1.0, 1.0, 1]];            
            };
		};
	...
	_highestId = 0;
    // Prodavec: -1 required
	for '_i' from 0 to (WFBE_Client_Teams_Count-1) do {if ((_voteArray select _i) > (_voteArray select _highestId)) then {_highestId = _i}}; //--- Get the most voted person.
	...
	if (time - _update_list > 1) then { //--- Refresh the list.
        ...
        // Prodavec: -1 required
		for '_i' from 0 to (WFBE_Client_Teams_Count-1) do { //--- Add potential new player controlled slots.

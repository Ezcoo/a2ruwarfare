...
if (!isNull(commanderTeam)) then {if (commanderTeam == group player) then {_realSize = _realSize*3}}; 
			...
			if !(_isInfantry) then {
				...
				if (_type != 'Depot') then {
					...
					if (typeName _slots == "ARRAY") then {
						...
						//--- do not allow uav's with crew
						if ([_listBox,_listUnits] call _Func_IsUav) then
						{
							_driver=false;
							_gunner=false;
							_commander=false;

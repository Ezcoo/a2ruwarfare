...
switch (_type) do {
	case "Land_Wall_L3_5m_EP1": {
		_defense addEventHandler ['handleDamage',{getDammage (_this select 0)+(_this select 2)/35}];
		_defense setDir _direction;
	};
	// case "Concrete_Wall_EP1": {_defense addEventHandler ['handleDamage',{getDammage (_this select 0)+(_this select 2)/140}]};
};

if (_type in ['Stinger_Pod','Stinger_Pod_US_EP1','Igla_AA_pod_East','Igla_AA_pod_TK_EP1']) then {
	_defense SetVehicleInit "this addeventhandler ['Fired',{_this spawn HandleMissiles;}];";  
         processInitCommands;
          (_defense) Spawn BalanceInit;
};
...
//--- Delete Buildings on death.
if (_defense isKindOf "Building") then {
	_defense addEventHandler ["killed", {(_this select 0) Spawn WFBE_SE_FNC_OnBuildingKilled}];
};
...
// Prodavec: don't equip resistance(GUER) AI with additional shit
if ((_isArtillery != -1) && (_side in WFBE_PRESENTSIDES)) then {[_defense,_isArtillery,_side] Call EquipArtillery};

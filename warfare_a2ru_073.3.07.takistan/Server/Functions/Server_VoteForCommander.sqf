...
// AI will be elected only by the unanimous vote
if (_aiVotes != _total) then {
	_aiVotes = 0;
};
...
if (isNull _commander) then {
    ...
    // Prodavec: added deploy status detection to prevent locality changing for deployed HQ
	if (!WF_A2_Vanilla && isMultiplayer && !(_side Call WFBE_CO_FNC_GetSideHQDeployStatus)) then {[_side Call WFBE_CO_FNC_GetSideHQ, leader _commander] Spawn WFBE_SE_FNC_SetLocalityOwner};
};
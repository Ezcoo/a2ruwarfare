...
BIS_CONTROL_CAM_Handler =
{
    ...
    // Prodavec
  	if ((isNil "_logic") || (isNil "BIS_CONTROL_CAM"))  exitWith {};
    //_camera = BIS_CONTROL_CAM;
...
while {!isNil "BIS_CONTROL_CAM"} do {
	...
	if (_mode in ["mousemoving","mouseholding"]) then {
		...
		if (count _params > 0) then {
			...
			if (typeof _preview != _itemclass_preview) then
            {
				...
				if (_index == 0 && _hqDeployed) exitWith {
                    ...
                    // Prodavec
                    _preview = if (isNil {_logic getVariable "BIS_COIN_preview"}) then {objNull} else {_logic getVariable "BIS_COIN_preview"};
        ...
        // Prodavec: default value is ""
        // For some shitty reason getVariable with default value doesn't work o_0
        _oldTooltip = if (isNil {_logic getVariable "BIS_COIN_tooltip"}) then {""} else {_logic getVariable "BIS_COIN_tooltip"};
        ...
        // Prodavec: shitty getVariable fix
        _restart = if (isNil {_logic getVariable "BIS_COIN_restart"}) then {false} else {_logic getVariable "BIS_COIN_restart"};

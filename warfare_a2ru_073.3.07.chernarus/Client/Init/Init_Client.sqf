...
//--- Custom MARTA Init.
ExecVM "Client\Init\Init_BIS_MARTA.sqf";
ExecVM "Client\Init\Init_BIS_HC.sqf";
...
if (isNil 'currentTG') then {
	currentTG = if ((missionNamespace getVariable "WFBE_C_ENVIRONMENT_MAX_CLUTTER") < 25) then {missionNamespace getVariable "WFBE_C_ENVIRONMENT_MAX_CLUTTER"} else {50};
...
[] Spawn {
	...
	Call Compile preprocessFileLineNumbers "Common\Config\Core_Upgrades\Labels_Upgrades.sqf";
	ExecVM "Common\Module\CIPHER\CIPHER_Sort.sqf";
};

//--- Prodavec: MapMarkersTitling script. (Show player names while placing markers).
execVM "Client\Module\Prodavec\MapMarkersTitling.sqf";
...
if ((missionNamespace getVariable "WFBE_C_ECONOMY_INCOME_SYSTEM") in [3,4]) then {
	if (isNil {WFBE_Client_Logic getVariable "wfbe_commander_percent"}) then {WFBE_Client_Logic setVariable ["wfbe_commander_percent", 15]};
};
...
/* Options menu. */
Options = player addAction ["<t color='#42b6ff'>" + (localize "STR_WF_Options") + "</t>","Client\Action\Action_Menu.sqf", "", -100, false, true, "", "_target == player"];
			...
			if (surfaceIsWater(position _preview)) then {
				...
				if (_color != _colorRed) then {
					if (_itemcategory == 0) then { //--- Structures.
						if (count(((position _preview) nearObjects ["Building", 0.000001]) - [_preview]) > 0) then {_color = _colorRed};
					} else { //--- Rest.
						if (count((position _preview) nearEntities [['Man','Car','Motorcycle','Tank','Air','Ship'], 0]) > 0) then {_color = _colorRed};
					};
				};
			};
...
//------	if !(typeOf _preview in ["Land_Wall_L3_5m_EP1"]) then {		
...
//--- General onFired EH.
player addEventHandler ['fired', {(_this) Spawn WFBE_CL_FNC_OnFired}];
...
12452 cutRsc  ["b2zgroup","BLACK IN",5];

...
while {true} do {
	...
	if (RUBHUD) then {
		...
		if (!isNull (["currentCutDisplay"] call BIS_FNC_GUIget)) then {
			...
			_textControl6 ctrlSetText Format ["+ %1", if( missionNamespace getVariable "WFBE_C_ECONOMY_INCOME_SYSTEM" == 3) then{round (0.75*(sideJoined Call GetTotalSupplyValue))}else{(sideJoined Call GetTotalSupplyValue)}]; //, sideJoined Call GetTownsHeld];

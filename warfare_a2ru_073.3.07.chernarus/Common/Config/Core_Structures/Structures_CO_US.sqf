...
/* Root Definition */
_MHQ = if (WF_Camo) then {'BMP2_HQ_CDF'} else {'BMP2_HQ_CDF'};
_HQ = if (WF_Camo) then {"BMP2_HQ_CDF_unfolded"} else {"BMP2_HQ_CDF_unfolded"};
...
_SP = if (WF_Camo) then {"USMC_WarfareBContructionSite1"} else {"US_WarfareBContructionSite1_Base_EP1"};
_AAR = if (WF_Camo) then {"USMC_WarfareBAntiAirRadar"} else {"US_WarfareBAntiAirRadar_EP1"};

/* Mash used after being deployed */
missionNamespace setVariable [Format["WFBE_%1FARP", _side], 'acamp'];
...
_c = _c		+ [200];
_t = _t		+ [if (WF_Debug) then {1} else {5}];
_s = _s		+ ["SmallSite"];
_dis = _dis	+ [18];
_dir = _dir	+ [90];

...
with missionNamespace do {
	WFBE_IRS_AREA_OPERATING = 35; //--- IR Smoke will operate only if the vehicle is within x meters of a SmokeLauncher grenade.
	WFBE_IRS_AUTO_DETECT_RANGE = 200; //--- IR Smoke missile smoke detection.
	WFBE_IRS_FLARE_DELAY = 60;
	WFBE_IRS_MISSILE_CHECK_RANGE = 200; //--- IR Smoke actions will be checked when the missile enter that range.
};

...
Private ["_get","_isMan","_logik","_side","_sideID","_unit","_unit_kind","_upgrades","_type"];
...
// Prodavec: unit init field fires much more earlier than commonInitComplete is initialized
// with false value. We have to wait for its initialization first.
waitUntil {sleep 0.117; (!isNil "commonInitComplete") && {commonInitComplete}}; //--- Wait for the common part.
...
if (_unit isKindOf "Tank" && _type != "BMP2_HQ_CDF" && _type != "BMP2_HQ_INS") then { //--- Tanks.
	//--- Valhalla Low gear.
...
if (_unit isKindOf "Su34") then { //--- Su39.
	//--- Valhalla Low gear.
	_unit addAction ["<t color='#FFBD4C'>"+(localize "STR_ACT_ForsageOn")+"</t>","Client\Module\Valhalla\LowGear_Toggle.sqf", [], 91, false, true, "", "(player==driver _target) && !Local_HighClimbingModeOn && canMove _target"];
	_unit addAction ["<t color='#FFBD4C'>"+(localize "STR_ACT_ForsageOff")+"</t>","Client\Module\Valhalla\LowGear_Toggle.sqf", [], 91, false, true, "", "(player==driver _target) && Local_HighClimbingModeOn && canMove _target"];
};

if (_unit isKindOf "BMP2_HQ_CDF") then { //--- BMP2_HQ_CDF.
	//--- Valhalla Low gear.
	_unit addAction ["<t color='#FFBD4C'>"+(localize "STR_ACT_TurboOn")+"</t>","Client\Module\Valhalla\LowGear_Toggle.sqf", [], 91, false, true, "", "(player==driver _target) && !Local_HighClimbingModeOn && canMove _target"];
	_unit addAction ["<t color='#FFBD4C'>"+(localize "STR_ACT_TurboOff")+"</t>","Client\Module\Valhalla\LowGear_Toggle.sqf", [], 91, false, true, "", "(player==driver _target) && Local_HighClimbingModeOn && canMove _target"];
};

if (_unit isKindOf "BMP2_HQ_INS") then { //--- BMP2_HQ_INS.
	//--- Valhalla Low gear.
	_unit addAction ["<t color='#FFBD4C'>"+(localize "STR_ACT_TurboOn")+"</t>","Client\Module\Valhalla\LowGear_Toggle.sqf", [], 91, false, true, "", "(player==driver _target) && !Local_HighClimbingModeOn && canMove _target"];
	_unit addAction ["<t color='#FFBD4C'>"+(localize "STR_ACT_TurboOff")+"</t>","Client\Module\Valhalla\LowGear_Toggle.sqf", [], 91, false, true, "", "(player==driver _target) && Local_HighClimbingModeOn && canMove _target"];
};

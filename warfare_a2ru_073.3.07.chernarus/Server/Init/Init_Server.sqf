...
WFBE_SE_FNC_OnBuildingKilled = Compile preprocessFileLineNumbers "Server\Functions\Server_OnBuildingKilled.sqf";
...
// Prodavec
waitUntil {(!isNil "commonInitComplete") && (!isNil "townInit") && {commonInitComplete} && {townInit}};
...
//--- Global sides initialization.
{
	...
	if (_x select 0) then {
		...
		if ((missionNamespace getVariable "WFBE_C_ECONOMY_INCOME_SYSTEM") in [3,4]) then {
			_logik setVariable ["wfbe_commander_percent", if ((missionNamespace getVariable "WFBE_C_ECONOMY_INCOME_PERCENT_MAX") < 15) then {missionNamespace getVariable "WFBE_C_ECONOMY_INCOME_PERCENT_MAX"} else {15}, true];
		};
...
if ((missionNamespace getVariable "WFBE_C_TOWNS_REINFORCEMENT_DEFENDER") > 0) then {
	Call Compile preprocessFile "Server\Module\Reinforcer\Init_Reinforcer.sqf"
};

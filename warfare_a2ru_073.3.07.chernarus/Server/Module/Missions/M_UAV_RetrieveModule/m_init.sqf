...
if (random 100 > 35) then {
	...
	//--- Create the team.
    // Prodavec: _list renamed to _teamType
	...
	_returned = [_teamType, _defensePos, resistance, if ((missionNamespace getVariable "WFBE_C_TOWNS_VEHICLES_LOCK_DEFENDER") > 0) then {true} else {false}, objNull, false] Call WFBE_CO_FNC_CreateTeam;
...
//--- Give the 'winning' side an award.
_winner = if (_ecomplete) then {east} else {west};
[_winner, "LocalizeMessage", ['SecondaryAwardSupply','Supply',_winner,1000]] Call WFBE_CO_FNC_SendToClients;
